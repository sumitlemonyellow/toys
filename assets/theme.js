...
Slate.A11yHelpers = (function () {
  ...
  A11yHelpers.prototype.removeTrapFocus = function (
    $container,
    eventNamespace
  ) {
    var eventName = eventNamespace ? "focusin." + eventNamespace : "focusin";
      ...
      $indicatorDots.each(function (index) {
        ...
        $element
          .attr({
            ...
            "aria-controls":
              "#" + $slides.filter(":not(.clone)").eq(index).attr("id"),
  ...
  theme.SlideshowSection.prototype,
  {
    ...
    onBlockDeselect: function () {
      ...
      if (
        ...
        !$slideshow.parent().find("[data-slider-pause]").hasClass("is-paused")
      ) {
...
theme.init = function () {
  ...
  theme.cache.$sortViewDropdown.on("click", theme.sortView);
...
theme.sortView = function () {
  let get_sort_btn = $(this).attr("sortv");

  console.log("get_sort_btn", get_sort_btn);
  $("#" + get_sort_btn).click();

  theme.variables.queryParams.sort_view = $("#" + get_sort_btn).val();

  location.search = Slate.QueryString.stringify(theme.variables.queryParams);
  //theme.cache.$sortViewDropdown.val(theme.cache.$sortViewDropdown.data('value'));

  if ($(".products").is(".block")) {
    $(".products").removeClass("block");
    $(".product-grid-row")
      .removeClass("row-cols-sm-2")
      .removeClass("row-cols-md-3");
  } else {
    $(".products").removeClass("list");
    $(".product-grid-row").addClass("row-cols-sm-2").addClass("row-cols-md-3");
  }

  $(".products").addClass(theme.variables.queryParams.sort_view);
...
theme.filterCollection = function () {
  ...
  if ($.isEmptyObject(theme.variables.queryParams)) {
    ...
    location.href =
      theme.cache.$tagsDropdown.val() +
      "?" +
      Slate.QueryString.stringify(theme.variables.queryParams);
  }
};
